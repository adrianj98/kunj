  ...
  async execute(options: TeamOptions = {}): Promise<void> {
    ...
    for (const pr of prs) {
      ...
      const unchanged =
        ...
        cached.headRefOid === pr.headRefOid &&
    ...
    if (useAI) {
      ...
      if (hasCredentials) {
        try {
          ...
          const reduceResult = await this.reduceGenerateReport(
            prs,
            summaries,
            recentActivity,
            jiraIssues,
          ...
          reportContent = reduceResult.reportContent;

          // Save structured analysis for UI
          const analysisDir = path.join(teamDir);
          const analysisData = {
            teamSummary: reduceResult.analysis.teamSummaryBullets,
            projects: reduceResult.analysis.projects,
            jiraByStatus: this.groupJiraByStatus(jiraIssues),
            slackMessages,
            prCount: prs.length,
            lastFetch: now,
          ...
          fs.writeFileSync(
            path.join(analysisDir, `analysis-${getTodayDate()}.json`),
            JSON.stringify(analysisData, null, 2),
            "utf8"
          );

          // Generate detailed project reports
          if (reduceResult.analysis.projects.length > 0) {
            console.log(chalk.blue("\nGenerating project reports..."));
            await this.generateProjectReports(
              reduceResult.analysis.projects,
              diffs,
              slackMessages,
              jiraIssues,
              newCache,
  ...
  private async fetchOpenPRs(limit: number): Promise<PRData[]> {
    try {
      const { stdout } = await execAsync(
        `gh pr list --json number,title,author,headRefName,headRefOid,baseRefName,isDraft,additions,deletions,createdAt,updatedAt,labels,url --state open --limit ${limit}`
      );
...
DISCUSSION: <summary of recent discussion if any, or "None">`;
  ...
  private async mapSummarizePRs(
    ...
    for (const pr of prs) {
      ...
      if (
        ...
        cacheEntry.headRefOid === pr.headRefOid
      ) {
  ...
  private async reduceGenerateReport(
  ...
  ): Promise<{ reportContent: string; analysis: import("../lib/team-analysis").TeamAnalysisResult }> {
    const authors = new Set(prs.map((pr) => pr.author.login));
    ...
    return { reportContent: report, analysis: result };
  }

  private groupJiraByStatus(jiraIssues: JiraIssueData[]): Record<string, JiraIssueData[]> {
    const byStatus: Record<string, JiraIssueData[]> = {};
  ...
  private async generateProjectReports(
    ...
    let cached = 0;
    const toGenerate: ParsedProject[] = [];

    for (const project of projects) {
      const slug = project.name.replace(/[^a-zA-Z0-9]/g, "-").toLowerCase();
      const reportPath = path.join(reportsDir, `${slug}-${date}.json`);

      // Build cache key from PR OIDs
      const cacheKey = project.prNumbers
        .sort((a, b) => a - b)
        .map(n => `${n}:${cache.prs[n]?.headRefOid || ''}`)
        .join("|");
      ...
      if (!force && fs.existsSync(reportPath)) {
        try {
          ...
          if (existing.cacheKey === cacheKey) {
            cached++;
            console.log(chalk.gray(`  ${project.name} — cached`));
            continue;
          }
        } catch {}
      }

      (project as any)._cacheKey = cacheKey;
      (project as any)._slug = slug;
      toGenerate.push(project);
    }

    if (toGenerate.length > 0) {
      console.log(
        chalk.gray(
          `  ${toGenerate.length} to generate, ${cached} cached (concurrency: ${CONCURRENCY})`
        )
      );
      ...
      for (let i = 0; i < toGenerate.length; i += CONCURRENCY) {
        ...
        console.log(
          chalk.gray(
            `  Batch: ${batch.map(p => p.name).join(", ")}`
          )
        );

        const results = await Promise.allSettled(
          batch.map(async (project) => {
            // Filter Jira to this project's tickets
            const projectJiraKeys = new Set(project.jiraTickets.map(j => j.key));
            const projectJira = jiraIssues.filter(i => projectJiraKeys.has(i.key));

            const report = await generateProjectReport(
              project, diffs, slackMessages, projectJira
            );
            report.cacheKey = (project as any)._cacheKey;
        ...
        for (const result of results) {
          if (result.status === "fulfilled") {
            ...
            const slug = (project as any)._slug;
            const reportPath = path.join(reportsDir, `${slug}-${date}.json`);
            fs.writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf8");
            console.log(chalk.gray(`  ✓ ${project.name}`));
          } else {
            console.log(
              chalk.yellow(`  ✗ Failed: ${result.reason?.message || "unknown error"}`)
            );
          }
        }
      }
    ...
    console.log(
      chalk.gray(
        `  ${projects.length} project reports (${cached} cached, ${toGenerate.length} generated)`
      )
    );
  }
